...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, CommonModule, ReactiveFormsModule, HeaderComponent, FooterComponent, RouterLinkActive],
...
export class RegisterComponent implements OnInit {
  ...
  countries = [
    ...
    { name: "Bosnia and Herzegowina", code: "+387" },
    ...
    { name: "Haiti", code: "+509" },
    { name: "Heard and Mc Donald Islands", code: "+61" },
  ...
  constructor(private fb: FormBuilder,
    ...
    this.miFormulario = this.fb.group({
      ...
      userEmailInput: ['', [Validators.required, Validators.email]],
      userCountryInput: ['',[ Validators.required]],  // Valor inicial vacío y requerido
      userTelInput: ['', [Validators.required]]
    });
  ...
  cambioPais(event: any):void {
    const codigoTel = document.getElementById('phoneCode') as HTMLInputElement;
    const valorSeleccionado = event.target.value;
    if(codigoTel) {
      const selectedCountry = this.countries.find(country => country.name === valorSeleccionado);
      if (selectedCountry) {
        codigoTel.value = selectedCountry.code;
      }
    }
  }
  guardarInformacion(): void {
    ...
    const informacionUsuario = {
      nombre: this.miFormulario.get('userNameInput')?.value,
      email: this.miFormulario.get('userEmailInput')?.value,
      pais: this.miFormulario.get('userCountryInput')?.value,
      telefono: this.miFormulario.get('userTelInput')?.value
    };
    console.table(informacionUsuario);
